...
function handleError(err) {
  if (!err) {
    err = new Error("Errore sconosciuto");
  }
...
function validateRow(row) {
  if (!row || typeof row !== 'object') {
    throw new Error("`row` non è un oggetto valido");
  }
}
...
function updatequotation(row) {
  try {
    ...
    const importance = ['Offerte.ID_Offerta', 'Anagrafica', 'Dettagli_Offerta', 'Offerte.Totale_offerta', 'invoicer', 'Offerte.Data_Offerta', 'Offerte.Netto_Offerta', 'Deduction', 'Offerte.VAT', 'Note'];
    ...
    if (!row.Offerte || !row.Offerte.Data_Offerta) {
      ...
      if (!seen.has('References') && !row.Offerte.Data_Offerta) {
...
ready(function() {
  ...
  grist.on('message', msg => {
    if (msg.tableId && !app.rowConnected) {
      ...
      }).catch(e => console.error('Errore durante il fetch della tabella:', e));
    }
