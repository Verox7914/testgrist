...
// Debug: Log dei dati ricevuti per analisi
...
if (Array.isArray(row.Items)) {
    row.Items = row.Items.map((item, index) => ({
        Description: item["Dettafli_Offerta.descrizione"] || `Item ${index + 1}`,
        ...
        Code: item["Dettafli_Offerta.Codice_Articolo"] || `Code ${index + 1}`,
    }));
}

// Debug: Log delle chiavi di Offerte per verifica
if (row["Offerte"]) {
    console.log("Offerte Data:", row["Offerte"]);
}

// Gestione dati mancanti per "Offerte"
row.Number = row["Offerte.ID_Offerta"] || "No ID";
row.Issued = row["Offerte.Data_Offerta"] || "No Date";
row.Due = row["Offerte.Data_Offerta"] || "No Due Date";
